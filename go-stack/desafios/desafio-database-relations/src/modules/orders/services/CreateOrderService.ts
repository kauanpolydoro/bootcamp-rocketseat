...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const existentProductsWithNoQuantityAvailable = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity,
    );

    if (existentProductsWithNoQuantityAvailable.length) {
      throw new AppError(
        `The quantity ${existentProductsWithNoQuantityAvailable[0].quantity} is not available for ${existentProductsWithNoQuantityAvailable[0].id}`,
      );
    }
